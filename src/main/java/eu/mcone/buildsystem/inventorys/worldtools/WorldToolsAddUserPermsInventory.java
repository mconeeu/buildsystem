...
public class WorldToolsAddUserPermsInventory extends CoreInventory {

    private int i = 0;
    private static final HashMap<Player, org.bukkit.World> World = new HashMap<>();
    ...
    private static final CoreAnvilInventory ANVIL_INVENTORY = CoreSystem.getInstance().createAnvilInventory(event -> {
        ...
        if (target != null && !target.equals(event.getPlayer())) {
            ...
            if (event.getSlot().equals(AnvilSlot.OUTPUT)) {
                ...
                buildtarger.promotePermissions(World.get(player), player);
                World.remove(player);
            }
        } else {
    ...
    public WorldToolsAddUserPermsInventory(Player player) {
